...
while (True):
    ...
    sat = cv2.cvtColor(frame, cv2.cv.CV_BGR2HSV)
    print type(sat)
    print sat
    # for i in sat.rows:
    #     for j in sat.cols:

    frame = cv2.cvtColor(frame, cv2.cv.CV_HSV2BGR)
    ...
    # img_size = gray.shape
    # print img_size
    gray = cv2.Canny(gray,1,3)
    # detect circles in the image
#    gray=cv2.bitwise_not(gray)
    circles = cv2.HoughCircles(gray, cv2.cv.CV_HOUGH_GRADIENT, 1.5, 200, param1=1, param2=75, minRadius=0, maxRadius=0)
    # print circles
