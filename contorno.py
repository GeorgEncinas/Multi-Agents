...
class Posicionamiento:
    ...
    def __init__(self):
        ...
        self.UPPER_RED = np.array([18,255,240])
    ...
    def obtener_mascara(self, image, color):
        ...
        if(color=='blue'):
            mascara = cv2.inRange(image_hsv, self.LOWER_BLUE, self.UPPER_BLUE)
            mascara = cv2.erode(mascara, (5,5))
            mascara = cv2.dilate(mascara, (5,5))
        elif(color == 'red'):
            mascara = cv2.inRange(image_hsv, self.LOWER_RED, self.UPPER_RED)
            mascara = cv2.erode(mascara, (5, 5))
            mascara = cv2.dilate(mascara, (5, 5))
        elif(color == 'blanco'):
    ...
    def obtener_contornos(self, mascara):
		contours, inheriters = cv2.findContours(mascara, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
		pos =[]
		for c in contours:
